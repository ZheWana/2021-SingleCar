...
uint8_t mainState = 0, subState = 0, Tflag = 0, delayFlag1 = 0, delayPreFlag1 = 0, delayFlag2, delayPreFlag2, turnFlag = 0, dottedFlag = 0, getCross = 0, getT = 0, crossState = 0, forceTurn = 0, dataFrame = 0;
double shadowDirOut = 0, basicLeftSpd = 0, basicRightSpd = 0;

extern uint8_t data, command, crossHistroy, crossCNT;
...
void ForceTurn(uint8_t dirMask) {
    ...
    switch (dirMask) {
        case DIR_BACK:
            ...
            while (TIM2->CNT - leftCntLoad < 26923);
            break;
        case DIR_LEFT:
            Set_Speed(0, MAXSpeed);
            while (TIM5->CNT - rightCntLoad < 26923);
            break;
        case DIR_RIGHT:
    ...
    if (huart->Instance == USART2) {
        ...
        do {
            if (command == '\n' || command == 'c') {
                ...
                switch (charflag) {
                    case 'c':
                        crossHistroy = (uint8_t) valint;//negtiveflag ? (-1) * (valint + valpoint) : (valint + valpoint);
                        charflag = 0;
                        break;
                    default:;
                }
            ...
            if (command == '-')negtiveflag = 1;
            if (command == '.')pointflag = 1;
            if (command <= '9' && command >= '0') {
                if (pointflag) {
                    valpoint = valpoint * 10 + (command - '0') * 1;
                } else {
                    valint = valint * 10 + (command - '0') * 1;
                }
            }
        } while (0);
    }
...
//    if (getCross)LED_ON(LED_RED | LED_YELLOW | LED_GREEN);
}
